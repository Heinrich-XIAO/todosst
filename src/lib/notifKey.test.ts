...
test("encryptNotifBlob/decryptNotifBlob roundtrip", async () => {
  ...
  const blob = await encryptNotifBlob(key, { name: "pay rent", min: 15 });
  expect(blob.iv).not.toBe("");
  expect(blob.ct).not.toBe("");
  expect(await decryptNotifBlob(key, blob)).toEqual({ name: "pay rent", min: 15 });
  expect(await decryptNotifBlob(key, await encryptNotifBlob(key, { k: "risk", streak: 12, missed: 0, open: 3 }))).toEqual({
    k: "risk",
    streak: 12,
    missed: 0,
    open: 3,
  });
  expect(await decryptNotifBlob(key, await encryptNotifBlob(key, "t — — x"))).toBe("t — — x");
});

test("decryptNotifBlob rejects a wrong key and a tampered blob", async () => {
  const key = generateNotifKeyB64();
  const blob = await encryptNotifBlob(key, { name: "secret task", min: 5 });
...
test("notifications never leak the task name to a reader without the key", async () => {
  const blob = await encryptNotifBlob(generateNotifKeyB64(), { name: "dentist appointment", min: 30 });
