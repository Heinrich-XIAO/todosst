...
- it records the day, so open task rows can escalate after two consecutive missed days ("missed twice — today is the one that matters", grey on the right of each open task). Misses are free; pairs aren't.
